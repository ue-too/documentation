[@ue-too/board-pixi-react-integration](../modules.md) / guide

# Getting Started

`@ue-too/board-pixi-react-integration` provides React-specific integration between the board camera system and PixiJS.

## Installation

```bash
npm install @ue-too/board-pixi-react-integration @ue-too/board pixi.js
```

## Basic Usage

```tsx
import { PixiBoardCanvas } from "@ue-too/board-pixi-react-integration";

function App() {
    return <PixiBoardCanvas width={800} height={600} />;
}
```
